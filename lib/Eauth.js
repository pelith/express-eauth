...
class Eauth {
      ...
      if (address) {
        if (ethUtil.isValidAddress(address)) {
          if (this.options.method === 'eth_signTypedData') {
            ...
            message = this.createTypedDataSignMessageV4(address)
          }
        ...
        if (this.options.method === 'eth_signTypedData') {
          ...
          recoveredAddress = this.confirmTypedDataSignMessageV4(message, signature)
        }
        else if (this.options.method === 'personal_sign') {
  ...
  constructSignTypeV4Data(message) {
    return {
      types: {
        EIP712Domain: [
  ...
  createTypedDataSignMessageV4(address) {
    const uuid = uuidv4()
    const message = crypto.createHmac('sha256', secret)
      .update(address + uuid)
      .digest('hex')

    const typedData = this.constructSignTypeV4Data(message)
  ...
  confirmTypedDataSignMessageV4(message, signature) {
    const typedData = this.constructSignTypeV4Data(message)
